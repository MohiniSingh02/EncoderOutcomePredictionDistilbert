...
def compute_thresholds_and_tuned_macro(pr_curve_results: list[tensor], num_labels: int) -> (tensor, dict[str, tensor]):
    precision_per_class = torch.empty(num_labels, device=pr_curve_results[0].device)
    recall_per_class = torch.empty(num_labels, device=pr_curve_results[0].device)
    f1_per_class = torch.empty(num_labels, device=pr_curve_results[0].device)
    thresholds = torch.empty(num_labels, device=pr_curve_results[0].device)
